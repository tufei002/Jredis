...
public class AtomicIntegerFieldUpdaterTest {
	 ...
	 * 其次访问对象不能是static类型的变量（因为在计算属性的偏移量的时候无法计算），也不能是final类型的变量（因为根本无法修改），
	 * 必须是普通的成员变量
	 * 
	 ...
	 */
	public final static AtomicIntegerFieldUpdater<A> ATOMIC_INTEGER_UPDATER = AtomicIntegerFieldUpdater
			.newUpdater(A.class, "intValue");
			...
			new Thread() {
				public void run() {
					...
					if (ATOMIC_INTEGER_UPDATER.compareAndSet(a, 100, 120)) {
						...
						System.out.println("我是线程：" + num + " error！");
					}
				}
			}.start();
		}
